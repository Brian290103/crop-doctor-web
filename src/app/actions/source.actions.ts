...
export const getSources = async (cropId?: string) => {
  ...
  const url = cropId ? `${BASE_URL}/api/sources?cropId=${cropId}` : `${BASE_URL}/api/sources`;
...
export const createSource = async (data: { cropId: string; title: string; content: string; type: string; metadata?: any }) => {
...
export const updateSource = async (id: string, data: { cropId?: string; title?: string; content?: string; type?: string; metadata?: any }) => {
