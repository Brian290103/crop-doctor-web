...
export async function POST(req: Request) {
  try {
    ...
    const existingCrop = await db.select().from(crops).where(eq(crops.id, cropId));
    if (existingCrop.length === 0) {
      return NextResponse.json({ error: "Crop with provided ID does not exist." }, { status: 404 });
    }

    const slug = getSlug(title); // Generate slug from title

    // 2. Check if slug is unique
    const existingSourceWithSlug = await db.select().from(sources).where(eq(sources.slug, slug));
    if (existingSourceWithSlug.length > 0) {
      return NextResponse.json({ error: "Source with this title (slug) already exists." }, { status: 409 });
    }
    ...
    } else if (error.issues) { // ZodError has an 'issues' property
